        ...
        <div className={styles.container}>
            ...
            {!showBrand && <div className={styles.horizonLine2}></div>}
                <div className={showBrand===true?styles.brandContentIsBlock:styles.brandContentIsNone}>
                    ...
                    {brands && brands.map((item,index) =><div className={styles.item} key={index}><label className={styles.itemName} > <input type="checkbox" checked={checkedBrandList.includes(item)?true:false}value={item} onChange={e =>onChecked(e,'brand')}/>{item}</label></div>)}
            ...
            <div className={styles.priceContainer} onClick={clickToPrice}>
                ...
                {showPrice ===false?<AiOutlinePlus className={styles.plus}/>:<AiOutlineMinus className={styles.plus}/>}
            </div>
            
            {!showPrice && <div className={styles.horizonLine2}></div>}
                <div className={showPrice===true?styles.brandContentIsBlock:styles.brandContentIsNone}>
                
                    {priceInitArr.map((item,index) =><div className={styles.item} key={index}><label className={styles.itemName} > <input type="checkbox" checked={checkedPriceList.includes(`${index+1}`)?true:false}value={item} onChange={e =>onChecked(e,'price',index+1)}/>{item}</label></div>)}
                    <div className={styles.horizonLine2}></div> 
                </div>
        </div>
    );
